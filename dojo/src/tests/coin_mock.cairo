...
mod coin_mock {
    ...
    fn dojo_init(ref self: ContractState) {
        self.erc20_metadata.initialize("Coin", "COIN", 18);
        self.initializable.initialize();
        // Give 1 $COIN to the initializer
        self.erc20_mintable.mint(get_caller_address(), WEI(1));
    }
    ...
    #[abi(embed_v0)]
    impl CoinMockFaucetImpl of super::ICoinMockFaucet<ContractState> {
        fn faucet(ref self: ContractState) {
            self.erc20_mintable.mint(get_caller_address(), WEI(1_000));
        }
